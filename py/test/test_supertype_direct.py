...
class TestSupertypeDirect:

    def test_should_direct_list_supertype(self):
        setup = _supertype_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-supertype", "live" if setup["live"] else "unit")
        if _skip:
...
def _supertype_direct_setup(mockres):

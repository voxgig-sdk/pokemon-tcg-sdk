...
class TestRarityDirect:

    def test_should_direct_list_rarity(self):
        setup = _rarity_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-rarity", "live" if setup["live"] else "unit")
        if _skip:
...
def _rarity_direct_setup(mockres):

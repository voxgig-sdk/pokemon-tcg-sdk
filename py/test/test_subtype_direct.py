...
class TestSubtypeDirect:

    def test_should_direct_list_subtype(self):
        setup = _subtype_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-subtype", "live" if setup["live"] else "unit")
        if _skip:
...
def _subtype_direct_setup(mockres):

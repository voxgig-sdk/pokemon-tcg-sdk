...
class PokemonTcgTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            PokemonTcgTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = PokemonTcgTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = PokemonTcgTestRunner.getenv("POKEMON_TCG_TEST_LIVE")
        override = PokemonTcgTestRunner.getenv("POKEMON_TCG_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = PokemonTcgTestRunner.getenv(key)
        ...
        explain = PokemonTcgTestRunner.getenv("POKEMON_TCG_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if PokemonTcgTestRunner._test_control is not None:
            return PokemonTcgTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                PokemonTcgTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            PokemonTcgTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = PokemonTcgTestRunner.load_test_control()
...
def load_env_local():
    PokemonTcgTestRunner.load_env_local()


def env_override(m):
    return PokemonTcgTestRunner.env_override(m)


def entity_data(v):
    return PokemonTcgTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return PokemonTcgTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return PokemonTcgTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return PokemonTcgTestRunner.load_test_control()


def live_delay_ms():
    return PokemonTcgTestRunner.live_delay_ms()

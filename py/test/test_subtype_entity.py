...
class TestSubtypeEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Subtype(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Subtype(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set POKEMON_TCG_TEST_SUBTYPE_ENTID JSON to run live")
        client = setup["client"]
...
def _subtype_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "POKEMON_TCG_TEST_SUBTYPE_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("POKEMON_TCG_TEST_SUBTYPE_ENTID"))

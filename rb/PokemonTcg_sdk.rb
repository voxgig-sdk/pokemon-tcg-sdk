...
require_relative 'PokemonTcg_types'
...
class PokemonTcgSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = PokemonTcgHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = PokemonTcgHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, PokemonTcgFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    PokemonTcgUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = PokemonTcgHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = PokemonTcgHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = PokemonTcgHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = PokemonTcgHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = PokemonTcgError.new(
        "graphql_error", "PokemonTcgSDK: graphql: #{msg}")
      res["graphql"] = errors
    end

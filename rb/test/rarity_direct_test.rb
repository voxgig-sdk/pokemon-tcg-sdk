...
class RarityDirectTest < Minitest::Test
  def test_direct_list_rarity
    setup = rarity_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-rarity", setup[:live] ? "live" : "unit")
    if _should_skip
...
def rarity_direct_setup(mockres)

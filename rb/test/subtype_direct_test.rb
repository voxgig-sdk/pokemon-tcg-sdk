...
class SubtypeDirectTest < Minitest::Test
  def test_direct_list_subtype
    setup = subtype_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-subtype", setup[:live] ? "live" : "unit")
    if _should_skip
...
def subtype_direct_setup(mockres)

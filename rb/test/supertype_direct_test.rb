...
class SupertypeDirectTest < Minitest::Test
  def test_direct_list_supertype
    setup = supertype_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-supertype", setup[:live] ? "live" : "unit")
    if _should_skip
...
def supertype_direct_setup(mockres)

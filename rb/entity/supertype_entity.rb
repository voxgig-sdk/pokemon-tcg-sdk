...
class SupertypeEntity
  ...
  def make
    opts = @_entopts.dup
    SupertypeEntity.new(@_client, opts)
  end
  ...
  # @return [Supertype, Hash] the current Supertype data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Supertype fields)
  def match_get
  ...
  #
  # @param reqmatch [SupertypeListMatch, Hash, nil] match filter (any subset of
  #   Supertype fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Supertype>, Array] the matching Supertype items; raises PokemonTcgError on failure

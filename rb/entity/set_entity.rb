...
class SetEntity
  ...
  def make
    opts = @_entopts.dup
    SetEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = PokemonTcgHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Set, Hash] the current Set data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = PokemonTcgHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Set fields)
  def match_get
  ...
  # Load a single Set.
  #
  # @param reqmatch [SetLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Set.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Set, Hash] the loaded Set; raises PokemonTcgError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = PokemonTcgHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [SetListMatch, Hash, nil] match filter (any subset of
  #   Set fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Set>, Array] the matching Set items; raises PokemonTcgError on failure

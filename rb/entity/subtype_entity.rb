...
class SubtypeEntity
  ...
  def make
    opts = @_entopts.dup
    SubtypeEntity.new(@_client, opts)
  end
  ...
  # @return [Subtype, Hash] the current Subtype data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Subtype fields)
  def match_get
  ...
  #
  # @param reqmatch [SubtypeListMatch, Hash, nil] match filter (any subset of
  #   Subtype fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Subtype>, Array] the matching Subtype items; raises PokemonTcgError on failure

...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        PokemonTcgSDK $client,
        PokemonTcgUtility $utility
    ): PokemonTcgContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new PokemonTcgContext($ctxmap, null);
    ...
    private static function fixctx(PokemonTcgContext $ctx, PokemonTcgSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?PokemonTcgError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new PokemonTcgError($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new PokemonTcgResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new PokemonTcgBaseFeature();
    ...
    {
        $hook_client = PokemonTcgSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends PokemonTcgBaseFeature {
            public $hook_fn = null;
            public function TestHook(PokemonTcgContext $ctx): void
            {
    ...
    {
        $init_client = PokemonTcgSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends PokemonTcgBaseFeature {
            public $init_fn = null;
    ...
    {
        $blocked_client = new PokemonTcgSDK([
            'system' => [
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new PokemonTcgSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = PokemonTcgSDK::test(null, $setup_opts);
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = PokemonTcgSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof PokemonTcgError) ? $result->err->msg : (string)$result->err;

...
{
    private static function utility(): PokemonTcgUtility
    {
        return new PokemonTcgUtility();
    }
    ...
    private static function ctx(array $over = []): PokemonTcgContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new PokemonTcgContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new PokemonTcgOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof PokemonTcgError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new PokemonTcgOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = PokemonTcgMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new PokemonTcgOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = PokemonTcgMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new PokemonTcgOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = PokemonTcgMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PokemonTcgSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = PokemonTcgMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new PokemonTcgResponse([]),
            'result' => new PokemonTcgResult([]),
        ]);
        [, $err] = PokemonTcgMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new PokemonTcgSpec([]),
            'response' => null,
            'result' => new PokemonTcgResult([]),
        ]);
        [, $err] = PokemonTcgMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new PokemonTcgSpec(['step' => 's']),
            'response' => new PokemonTcgResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new PokemonTcgResult([]),
        ]);
        [, $err] = PokemonTcgMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new PokemonTcgSpec(['step' => 's']),
            'response' => new PokemonTcgResponse(self::resp(200, ['v' => 1])),
            'result' => new PokemonTcgResult([]),
        ]);
        [, $err] = PokemonTcgMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new PokemonTcgSpec(['step' => 's']),
            'response' => new PokemonTcgResponse(self::resp(200, ['v' => 2])),
            'result' => new PokemonTcgResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        PokemonTcgMakeResponse::call($ctx);
    ...
    {
        $preset = new PokemonTcgResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new PokemonTcgSpec([]),
            'response' => new PokemonTcgResponse([]),
            'result' => new PokemonTcgResult([]),
        ...
        [$got, $err] = PokemonTcgMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new PokemonTcgResult([])]);
        [, $err] = PokemonTcgMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new PokemonTcgSpec([]), 'result' => null]);
        [, $err] = PokemonTcgMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PokemonTcgSpec(['step' => 's']),
            'result' => new PokemonTcgResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new PokemonTcgOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PokemonTcgMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PokemonTcgSpec(['step' => 's']),
            'result' => new PokemonTcgResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new PokemonTcgOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PokemonTcgMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PokemonTcgResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new PokemonTcgSpec([]),
            'result' => new PokemonTcgResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = PokemonTcgMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PokemonTcgMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new PokemonTcgError('boom', 'boom');
        $utility->fetcher = function (PokemonTcgContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PokemonTcgSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = PokemonTcgMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (PokemonTcgContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = PokemonTcgMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(PokemonTcgResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        PokemonTcgMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (PokemonTcgContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PokemonTcgSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = PokemonTcgMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new PokemonTcgResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new PokemonTcgSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = PokemonTcgMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = PokemonTcgMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = PokemonTcgMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new PokemonTcgResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, PokemonTcgDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new PokemonTcgResult(['ok' => false])]);
        $this->expectException(PokemonTcgError::class);
        PokemonTcgDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new PokemonTcgResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', PokemonTcgMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        PokemonTcgMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new PokemonTcgBaseFeature();
        $b = new PokemonTcgBaseFeature();
        PokemonTcgFeatureAdd::call($ctx, $a);
        PokemonTcgFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): PokemonTcgBaseFeature
    {
        $f = new PokemonTcgBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        PokemonTcgFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): PokemonTcgContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new PokemonTcgSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PokemonTcgPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        PokemonTcgPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new PokemonTcgResponse(['headers' => null]),
            'result' => new PokemonTcgResult([]),
        ]);
        PokemonTcgResultHeaders::call($ctx);

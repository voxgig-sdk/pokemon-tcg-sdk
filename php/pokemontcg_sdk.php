...
class PokemonTcgSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = PokemonTcgHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = PokemonTcgHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!PokemonTcgFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, PokemonTcgFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return PokemonTcgUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = PokemonTcgHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = PokemonTcgHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = PokemonTcgHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new PokemonTcgError($op . "_allow",
                "PokemonTcgSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = PokemonTcgHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new PokemonTcgError("graphql_error",
                "PokemonTcgSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    // Canonical facade: $client->Rarity()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->rarity()
    // resolves here too.
    public function Rarity($data = null)
    {
        require_once __DIR__ . '/entity/rarity_entity.php';
        if ($data === null) {
            if ($this->_rarity === null) {
                $this->_rarity = new RarityEntity($this, null);
            }
            return $this->_rarity;
        }
        return new RarityEntity($this, $data);
    }

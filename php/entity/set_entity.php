...
class SetEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new SetEntity($this->_client, $opts);
    }

    /**
     * @param Set|array $args Set data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = PokemonTcgHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Set|array The current Set data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Set fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = PokemonTcgHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Set fields).
     */
     ...
     * Load a single Set.
     *
     * @param SetLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed SetLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Set|array The loaded Set as an assoc-array at the
     *   SDK boundary; throws PokemonTcgError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = PokemonTcgHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List Set items matching the given filter.
     *
     * @param SetListMatch|array|null $reqmatch Match filter (any subset
     *   of Set fields) as an assoc-array; SetListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Set[]|array A list of Set items as assoc-arrays at
     *   the SDK boundary; throws PokemonTcgError on failure (item-5 convention).
     */

...
describe('SubtypeDirect', async () => {
  ...
  test('direct-list-subtype', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-subtype', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['POKEMON_TCG_TEST_SUBTYPE_ENTID']

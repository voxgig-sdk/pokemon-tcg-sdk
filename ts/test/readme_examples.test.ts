...
//     (\`new PokemonTcgSDK(...)\` / \`PokemonTcgSDK.test(...)\` become
//     \`PokemonTcgSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"card":{"test01":{"id":"test01"}},"rarity":{"test01":{"id":"test01"}},"set":{"test01":{"id":"test01"}},"subtype":{"test01":{"id":"test01"}},"supertype":{"test01":{"id":"test01"}},"type":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new PokemonTcgSDK(...)\` and any
// \`PokemonTcgSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+PokemonTcgSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/PokemonTcgSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

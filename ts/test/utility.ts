...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.POKEMONTCG_TEST_LIVE ||
    'TRUE' === process.env.POKEMONTCG_TEST_OVERRIDE
  ) {
  ...
  m.POKEMONTCG_TEST_EXPLAIN = process.env.POKEMONTCG_TEST_EXPLAIN || m.POKEMONTCG_TEST_EXPLAIN

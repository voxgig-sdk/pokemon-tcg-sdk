...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.POKEMON_TCG_TEST_LIVE ||
    'TRUE' === process.env.POKEMON_TCG_TEST_OVERRIDE
  ) {
  ...
  m.POKEMON_TCG_TEST_EXPLAIN = process.env.POKEMON_TCG_TEST_EXPLAIN || m.POKEMON_TCG_TEST_EXPLAIN

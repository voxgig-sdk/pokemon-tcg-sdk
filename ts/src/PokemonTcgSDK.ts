...
class PokemonTcgSDK {
  ...
  async direct(fetchargs?: any) {
    if (!this._options.allow.op.includes('direct')) {
      return {
        ok: false,
        err: new Error('PokemonTcgSDK: direct: operation not allowed by' +
  ...
  async graphql(query: string, variables?: any, ctrl?: any) {
    ...
    if (!options.allow.op.includes('graphql')) {
      return {
        ok: false,
        err: new Error('PokemonTcgSDK: graphql: operation not allowed by' +
    ...
    if (null != errors && Array.isArray(errors) && 0 < errors.length) {
      ...
      const err: any = new Error('PokemonTcgSDK: graphql: ' +
  ...
  // Entity access: `client.Rarity().list()` / `client.Rarity().load({ id })`.
  // The argument is the entity OPTIONS object (passed to the entity
  // constructor as entopts), not initial entity data.
  Rarity(entopts?: Record<string, any>) {
    const self = this
    return new RarityEntity(self, entopts)
  }
  ...
  // The argument is the entity OPTIONS object (passed to the entity
  // constructor as entopts), not initial entity data.
  Subtype(entopts?: Record<string, any>) {
    const self = this
    return new SubtypeEntity(self, entopts)
  }
  ...
  // The argument is the entity OPTIONS object (passed to the entity
  // constructor as entopts), not initial entity data.
  Supertype(entopts?: Record<string, any>) {
    const self = this
    return new SupertypeEntity(self, entopts)
  }

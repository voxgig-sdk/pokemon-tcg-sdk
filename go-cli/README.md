...
boru-driven command-line client **and** interactive REPL for the PokemonTcg
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/pokemon-tcg-cli)
make build

# 2. See usage (words, entities, env vars)
./pokemon-tcg-cli --help

# 3. Provide credentials once, via the environment
export POKEMON_TCG_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./pokemon-tcg-cli list card
./pokemon-tcg-cli load 1 card            # {id:1} shorthand
./pokemon-tcg-cli load '{id:1}' card       # explicit match map
./pokemon-tcg-cli list rarity

# 5. Override the API base URL for a single call
POKEMON_TCG_BASE=https://api.example.com ./pokemon-tcg-cli list card

# 6. No arguments -> interactive REPL
./pokemon-tcg-cli
pokemon-tcg> list card
pokemon-tcg> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/pokemon-tcg-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/pokemon-tcg-cli list card
   ```
...
```sh
./pokemon-tcg-cli load 1 card          # scalar shorthand for {id:1}
./pokemon-tcg-cli load '{id:1}' card     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `pokemon-tcg>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/pokemon-tcg-cli`. |
...
(`./pokemon-tcg-cli <expr>`) and interactively (the REPL), and expressions compose the

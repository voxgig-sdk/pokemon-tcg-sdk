// Command PokemonTcg-cli is a CLI + REPL for the PokemonTcg SDK,
...
const prompt = "pokemon-tcg"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "card rarity set subtype supertype type"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      POKEMON_TCG_APIKEY (api key), POKEMON_TCG_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: POKEMON_TCG_APIKEY carries the API key and
	// POKEMON_TCG_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("POKEMON_TCG_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("POKEMON_TCG_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewPokemonTcgSDK(opts)

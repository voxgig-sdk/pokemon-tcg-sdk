...
[MCP](https://modelcontextprotocol.io) server exposing the PokemonTcg SDK as
two agent tools — `pokemon-tcg_list` and `pokemon-tcg_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/pokemon-tcg-mcp)
make build

# 2. Provide credentials via the environment
export POKEMON_TCG_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user pokemon-tcg \
  -- /absolute/path/to/pokemon-tcg-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./pokemon-tcg-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// pokemon-tcg_list: first page of records
{ "entity": "card" }
{ "entity": "card", "query": { } }

// pokemon-tcg_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/pokemon-tcg-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user pokemon-tcg \
     -- "$PWD"/dist/*/pokemon-tcg-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `pokemon-tcg_list` and `pokemon-tcg_load` tools now appear
   in new sessions. Ask the agent to *"list card using pokemon-tcg"*
   and it calls `pokemon-tcg_list` with `{"entity":"card"}`.
...
```sh
export POKEMON_TCG_APIKEY=sk_live_xxx            # API key
export POKEMON_TCG_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `pokemon-tcg_list` tool
...
### Call the `pokemon-tcg_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `pokemon-tcg_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `pokemon-tcg_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `POKEMON_TCG_APIKEY` | API key sent with every request. |
| `POKEMON_TCG_BASE` | Optional override of the API base URL. |

### Entities

The 6 entities valid as the `entity` argument:

card | rarity | set | subtype | supertype | type

### Smoke test via HTTP (raw JSON-RPC)

```sh
./pokemon-tcg-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"pokemon-tcg_load","arguments":{"entity":"card","query":{"id":1}}}}'
```

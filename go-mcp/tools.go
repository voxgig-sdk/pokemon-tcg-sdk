...
type Args struct {
	Entity string         `json:"entity" jsonschema:"card | rarity | set | subtype | supertype | type"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.PokemonTcgSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "pokemon-tcg_list",
		Description: "List records from PokemonTcg. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "pokemon-tcg_load",
		Description: "Load a single record from PokemonTcg. " +
...
func runOp(client *sdk.PokemonTcgSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.PokemonTcgSDK, name string) (sdk.PokemonTcgEntity, error) {
	switch strings.ToLower(name) {
	case "card":
		return client.Card(nil), nil
	case "rarity":
		return client.Rarity(nil), nil
	case "set":
		return client.Set(nil), nil
	case "subtype":
		return client.Subtype(nil), nil
	case "supertype":
		return client.Supertype(nil), nil
	case "type":

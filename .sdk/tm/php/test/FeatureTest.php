...
class FtCtrl extends PokemonTcgControl
{
...
class FtClient
{
    ...
    private PokemonTcgUtility $utility;

    public function __construct(PokemonTcgUtility $utility, array $options)
    {
    ...
    public function get_utility(): PokemonTcgUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PokemonTcgContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public PokemonTcgContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PokemonTcgContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PokemonTcgConfig::make_feature($name);
    ...
    private static function buildUrl(PokemonTcgSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PokemonTcgError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PokemonTcgResponse($fetched);
        }
    ...
    private function fail(PokemonTcgContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PokemonTcgContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

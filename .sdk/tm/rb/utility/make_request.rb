...
module PokemonTcgUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = PokemonTcgResponse.new({})
    ...
    if fetch_err
      ...
      response = PokemonTcgResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = PokemonTcgResponse.new(fetched)
    else

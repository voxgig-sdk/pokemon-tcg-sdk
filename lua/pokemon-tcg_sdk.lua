...
local PokemonTcgSDK = {}
PokemonTcgSDK.__index = PokemonTcgSDK
...
PokemonTcgSDK._make_feature = _make_feature


function PokemonTcgSDK.new(options)
  local self = setmetatable({}, PokemonTcgSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function PokemonTcgSDK:options_map()
...
function PokemonTcgSDK:get_utility()
  return Utility.copy(self._utility)
end


function PokemonTcgSDK:get_root_ctx()
  return self._rootctx
end


function PokemonTcgSDK:prepare(fetchargs)
...
function PokemonTcgSDK:direct(fetchargs)
...
-- by passing it.
function PokemonTcgSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function PokemonTcgSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "PokemonTcgSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function PokemonTcgSDK:Rarity(data)
  ...
  if data == nil then
    if self._rarity == nil then
      self._rarity = EntityMod.new(self, nil)
    end
    return self._rarity
  end
  return EntityMod.new(self, data)
...
function PokemonTcgSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = PokemonTcgSDK.new(sdkopts)
  sdk.mode = "test"

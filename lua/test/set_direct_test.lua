...
describe("SetDirect", function()
  it("should direct-list-set", function()
    local setup = set_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-set", setup.live and "live" or "unit")
  ...
  it("should direct-load-set", function()
    local setup = set_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-set", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function set_direct_setup(mockres)
  runner.load_env_local()

  local calls = {}

  local env = runner.env_override({
    ["POKEMON_TCG_TEST_SET_ENTID"] = {},

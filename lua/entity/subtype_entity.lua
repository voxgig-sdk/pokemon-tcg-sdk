...
local SubtypeEntity = {}
SubtypeEntity.__index = SubtypeEntity


function SubtypeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SubtypeEntity)
  self._name = "subtype"
  self._client = client
...
function SubtypeEntity:get_name()
  return self._name
end


function SubtypeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SubtypeEntity.new(self._client, opts)
end
...
function SubtypeEntity:mark_deleted()
  self._deleted = true
end


function SubtypeEntity:deleted()
  return true == self._deleted
end


function SubtypeEntity:data_set(args)
  if args ~= nil then
...
function SubtypeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SubtypeEntity:match_set(args)
  if args ~= nil then
...
function SubtypeEntity:match_get()
...
function SubtypeEntity:stream(action, args, callopts)
...
---@param reqmatch SubtypeListMatch
---@param ctrl? table
---@return Subtype[]
---@return string? err
function SubtypeEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function SubtypeEntity:_run_op(ctx, post_done)
  local utility = self._utility

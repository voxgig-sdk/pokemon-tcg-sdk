...
local RarityEntity = {}
RarityEntity.__index = RarityEntity


function RarityEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RarityEntity)
  self._name = "rarity"
  self._client = client
...
function RarityEntity:get_name()
  return self._name
end


function RarityEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RarityEntity.new(self._client, opts)
end
...
function RarityEntity:mark_deleted()
  self._deleted = true
end


function RarityEntity:deleted()
  return true == self._deleted
end


function RarityEntity:data_set(args)
  if args ~= nil then
...
function RarityEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RarityEntity:match_set(args)
  if args ~= nil then
...
function RarityEntity:match_get()
...
function RarityEntity:stream(action, args, callopts)
...
function RarityEntity:_run_op(ctx, post_done)
  local utility = self._utility

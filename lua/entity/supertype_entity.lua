...
local SupertypeEntity = {}
SupertypeEntity.__index = SupertypeEntity


function SupertypeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SupertypeEntity)
  self._name = "supertype"
  self._client = client
...
function SupertypeEntity:get_name()
  return self._name
end


function SupertypeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SupertypeEntity.new(self._client, opts)
end
...
function SupertypeEntity:mark_deleted()
  self._deleted = true
end


function SupertypeEntity:deleted()
  return true == self._deleted
end


function SupertypeEntity:data_set(args)
  if args ~= nil then
...
function SupertypeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SupertypeEntity:match_set(args)
  if args ~= nil then
...
function SupertypeEntity:match_get()
...
function SupertypeEntity:stream(action, args, callopts)
...
---@param reqmatch SupertypeListMatch
---@param ctrl? table
---@return Supertype[]
---@return string? err
function SupertypeEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function SupertypeEntity:_run_op(ctx, post_done)
  local utility = self._utility

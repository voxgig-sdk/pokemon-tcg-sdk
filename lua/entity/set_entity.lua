...
local SetEntity = {}
SetEntity.__index = SetEntity


function SetEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SetEntity)
  self._name = "set"
  self._client = client
...
function SetEntity:get_name()
  return self._name
end


function SetEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SetEntity.new(self._client, opts)
end
...
function SetEntity:mark_deleted()
  self._deleted = true
end


function SetEntity:deleted()
  return true == self._deleted
end


function SetEntity:data_set(args)
  if args ~= nil then
...
function SetEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SetEntity:match_set(args)
  if args ~= nil then
...
function SetEntity:stream(action, args, callopts)
...
function SetEntity:_run_op(ctx, post_done)
  local utility = self._utility

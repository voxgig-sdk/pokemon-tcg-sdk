...
func (sdk *PokemonTcgSDK) OptionsMap() map[string]any {
...
func (sdk *PokemonTcgSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *PokemonTcgSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *PokemonTcgSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *PokemonTcgSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *PokemonTcgSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *PokemonTcgSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("PokemonTcgSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *PokemonTcgSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *PokemonTcgSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("PokemonTcgSDK: graphql: %s", msg)
		res["graphql"] = errors
	}

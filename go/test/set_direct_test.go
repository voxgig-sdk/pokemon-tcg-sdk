...
func TestSetDirect(t *testing.T) {
	t.Run("direct-list-set", func(t *testing.T) {
		setup := setDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-set", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-set", func(t *testing.T) {
		setup := setDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-set", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type setDirectSetupResult struct {
	client *sdk.PokemonTcgSDK
	calls  *[]map[string]any
	live   bool
	idmap  map[string]any
}

func setDirectSetup(mockres any) *setDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["POKEMON_TCG_TEST_SET_ENTID"]; ok {
		...
		return &setDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &setDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}

...
func TestSupertypeEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Supertype(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Supertype(nil).Stream("list", nil, nil) {
...
func supertypeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("POKEMON_TCG_TEST_SUPERTYPE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["POKEMON_TCG_TEST_SUPERTYPE_ENTID"])
